import{n as e}from"./vendor-BuoCFfzO.js";import{t}from"./wii-B7KacVyR.js";import{p as n,r,u as i}from"./widev-BWCA_pqe.js";import{D as a,T as o,h as s,x as c}from"./firebase-CTU0UJ6d.js";import{n as l}from"./firebase-DNXHJgUE.js";var u=()=>i(`wiSmile`),d=`amUsersTotal`,f=`amUsersPro`,p=`amOrgsTotal`,m=`amLessonsTotal`,h=()=>{let e=u();return!e||e.rol!==`admin`?`<div class="am_page"><div class="am_empty"><i class="fas fa-ban"></i> Acceso denegado.</div></div>`:`
  <div class="am_page">
  ...
  </div>`},g=async()=>{let t=u();!t||t.rol!==`admin`||(e(document).off(`.am`),v(),y(),e(document).on(`click.am`,`#am_btn_refresh`,function(){let t=e(this).find(`i`).addClass(`fa-spin`);y(!0).then(()=>{setTimeout(()=>t.removeClass(`fa-spin`),500)})}))},_=()=>{e(document).off(`.am`)};function v(){e(`#am_n_total`).text(i(d)||`—`),e(`#am_n_pro`).text(i(f)||`—`),e(`#am_n_org`).text(i(p)||`—`),e(`#am_n_les`).text(i(m)||`45`)}async function y(t=!1){try{let u=a(l,`smiles`);if(t||!i(d)){let t=(await s(u)).data().count;n(d,t),e(`#am_n_total`).text(t)}if(t||!i(f)){let t=(await s(c(u,o(`rol`,`in`,[`gestor`,`empresa`,`admin`])))).data().count;n(f,t),e(`#am_n_pro`).text(t)}if(t||!i(p)){let t=(await s(c(u,o(`rol`,`==`,`empresa`)))).data().count;n(p,t),e(`#am_n_org`).text(t)}t&&r(`Estadísticas sincronizadas`,`success`)}catch(e){console.error(`[Admin] Error cargando stats:`,e),t&&r(`Error sincronizando`,`error`)}}export{_ as cleanup,g as init,h as render};