...
`,C=null,w=null,T=60;function E(){w&&clearInterval(w);let e=localStorage.getItem(`vault_expire`);e||(e=Date.now()+T*1e3,localStorage.setItem(`vault_expire`,e));let r=()=>Math.max(0,Math.ceil((parseInt(e)-Date.now())/1e3)),i=r();t(`#vault_timer`).text(`${i}s`);let o=async()=>{if(i=r(),t(`#vault_timer`).text(`${i}s`),i<=0){w&&clearInterval(w),w=null,localStorage.removeItem(`vault_expire`),n(`Sesión cerrada por inactividad`,`error`);let{salir:e}=await a(async()=>{let{salir:e}=await import(`./login-w5_3p2Uv.js`);return{salir:e}},__vite__mapDeps([0]));await e()}};o(),i>0&&(w=setInterval(o,1e3))}var D=()=>{let e=r(`wiSmile`);return!e||e.rol!==`admin`?``:`
  ...
  `},O=async()=>{t(`body`).addClass(`is-vault-locked`),E(),t(document).off(`.vault_shield`),t(document).on(`click.vault_shield`,`a, [href], .nv_item`,function(e){if(t(`body`).hasClass(`is-vault-locked`)){let r=t(this);if(r.attr(`id`)===`btn_vault_back`||r.closest(`#btn_vault_back`).length)return;e.preventDefault(),e.stopPropagation(),n(`<i class="fas fa-exclamation-triangle"></i> Identidad no verificada. Completa el 2FA primero.`,`warning`)}}),t(document).on(`contextmenu.vault_shield`,function(e){t(`body`).hasClass(`is-vault-locked`)&&(e.preventDefault(),n(`<i class="fas fa-eye-slash"></i> Clic derecho inhabilitado por seguridad.`,`warning`))}),t(document).on(`copy.vault_shield cut.vault_shield paste.vault_shield`,`input, body`,function(e){t(`body`).hasClass(`is-vault-locked`)&&(e.preventDefault(),n(`<i class="fas fa-key"></i> Copiar y pegar inhabilitado en esta boveda.`,`warning`))}),t(document).on(`keydown.vault_shield`,function(e){if(t(`body`).hasClass(`is-vault-locked`)){if(e.keyCode===123)return e.preventDefault(),n(`<i class="fas fa-shield-alt"></i> DevTools bloqueado por seguridad.`,`error`),!1;if(e.ctrlKey||e.metaKey){let t=String.fromCharCode(e.keyCode).toLowerCase();if(t===`u`||t===`s`||t===`p`||e.shiftKey&&(t===`i`||t===`j`||t===`c`))return e.preventDefault(),n(`<i class="fas fa-shield-alt"></i> Combinación de teclas restringida en esta zona.`,`error`),!1}}});let e=y(await p());if(e)try{let n=await l(u(d,`configwii`,e.usuario)),r=n.exists()?n.data():null;r?.configurado&&r?.secret?(C=r.secret,t(`#vault_card_container`).removeClass(`vault_card_setup`),t(`#vault_content_area`).html(S),A(e)):(t(`#vault_card_container`).addClass(`vault_card_setup`),t(`#vault_content_area`).html(x),await k(e))}catch(e){console.error(`[verificar] init:`,e),n(`Error al cargar la bóveda`,`error`)}};async function k(i){let o=await a(()=>import(`./vendor-BuoCFfzO.js`).then(t=>e(t.t(),1)),[]);C=g();let l=`WiiBlock`,f=r(`wiSmile`),p=`otpauth://totp/${l}:${encodeURIComponent(f?.usuario||i.usuario)}?secret=${C}&issuer=${l}&algorithm=SHA1&digits=6&period=30`;await o.toCanvas(document.getElementById(`vault_qr`),p,{width:200,margin:2,color:{dark:`#0f172a`,light:`#ffffff`}}),t(document).on(`input.vault`,`#vault_code_setup`,function(){this.value=this.value.replace(/[^0-9]/g,``),t(`#btn_vault_confirmar`).prop(`disabled`,this.value.length!==6)}),t(document).on(`click.vault`,`#btn_vault_confirmar`,async function(){let e=t(`#vault_code_setup`).val().trim();if(e.length!==6)return;if(!await v(e,C)){n(`<i class="fas fa-times-circle"></i> Código incorrecto, intenta de nuevo`,`error`),t(`#vault_code_setup`).val(``).focus();return}let r=t(this);r.html(`<i class="fas fa-spinner fa-spin"></i> Guardando...`).prop(`disabled`,!0);try{await c(u(d,`configwii`,i.usuario),{configurado:!0,secret:C,email:i.email||``,creado:s(),actualizado:s()}),n(`<i class="fas fa-lock"></i> ¡Bóveda configurada! Bienvenido al panel.`,`success`),j()}catch(e){console.error(`[verificar] setup save:`,e),n(`Error al guardar la configuración`,`error`),r.html(`<i class="fas fa-lock"></i> Confirmar y Cerrar Puerta`).prop(`disabled`,!1)}})}function A(e){setTimeout(()=>t(`#vault_code`).focus(),100),t(document).on(`input.vault`,`#vault_code`,function(){this.value=this.value.replace(/[^0-9]/g,``),this.value.length===6&&t(`#btn_code`).click()}),t(document).on(`click.vault`,`#btn_code`,async function(){let e=t(`#vault_code`).val().trim();if(e.length!==6)return n(`Ingresa los 6 dígitos`,`warning`);let r=t(this),i=r.html();r.html(`<i class="fas fa-spinner fa-spin"></i> Verificando...`).prop(`disabled`,!0);try{await v(e,C)?(n(`<i class="fas fa-unlock"></i> ¡Bóveda desbloqueada!`,`success`),j()):(n(`<i class="fas fa-times-circle"></i> Código incorrecto o expirado`,`error`),t(`#vault_code`).val(``).focus(),r.html(i).prop(`disabled`,!1))}catch(e){console.error(`[verificar] unlock:`,e),r.html(i).prop(`disabled`,!1)}}),t(document).on(`click.vault`,`#btn_vault_back`,()=>{localStorage.removeItem(`vault_expire`),i.navigate(`/`)})}function j(){w&&=(clearInterval(w),null),localStorage.removeItem(`vault_expire`),t(`body`).removeClass(`is-vault-locked`),sessionStorage.setItem(`vault_unlocked`,`true`),window.location.href=`/admin`}var M=()=>{w&&=(clearInterval(w),null),t(`body`).removeClass(`is-vault-locked`),t(document).off(`.vault`),t(document).off(`.vault_shield`)};export{M as cleanup,O as init,D as render};