...
export const render = () => {
  ...
  if (!user || (user.rol !== 'gestor' && user.rol !== 'admin')) {
    return `<div class="g_page"><div class="g_empty"><i class="fas fa-ban"></i> Acceso denegado.</div></div>`;
  }
    ...
    <div class="g_wrap">
      ...
      <header class="g_header wi_fadeUp">
        <div class="g_header_left">
          ...
          <div class="g_welcome">
            <h1>Panel Gestor</h1>
            <p>Bienvenido, <strong>${user.nombre || user.usuario}</strong>. Monitorea las operaciones y ventas en tiempo real.</p>
          </div>
        </div>
      </header>
