...
Escríbenos tu consulta y te responderemos en minutos. 👨‍💻`}},d=`caras`,f=[],p=()=>i(`wiSmile`)||{},m=()=>{try{return JSON.parse(localStorage.getItem(s)||`[]`)}catch{return[]}},h=e=>{localStorage.setItem(s,JSON.stringify(e.slice(0,32)))},g=e=>{f=m(),f=f.filter(t=>t!==e),f.unshift(e),f=f.slice(0,32),h(f)},_=()=>localStorage.getItem(o)||``,v=e=>localStorage.setItem(o,e),y=async e=>{try{return await navigator.clipboard.writeText(e),!0}catch{let t=document.createElement(`textarea`);return t.value=e,t.style.position=`fixed`,t.style.opacity=`0`,document.body.appendChild(t),t.select(),document.execCommand(`copy`),document.body.removeChild(t),!0}},b=(t,n,r)=>{let i=e(`body`);for(let a=0;a<8;a++){let a=Math.random()*160-80,o=Math.random()*-180-60,s=Math.random()*720-360,c=Math.random()*10+18,l=e(`<span class="em_particle"></span>`).text(t).css({left:n+`px`,top:r+`px`,fontSize:c+`px`,"--tx":`${a}px`,"--ty":`${o}px`,"--rot":`${s}deg`});i.append(l),setTimeout(()=>l.remove(),1e3)}},x=e=>e?e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/\n/g,`<br>`).replace(/\*(.*?)\*/g,`<strong>$1</strong>`).replace(/_(.*?)_/g,`<em>$1</em>`).replace(/~(.*?)~/g,`<del>$1</del>`):`Tu mensaje se mostrará aquí... 😊`,S=e=>e?e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/\n/g,`<br>`).replace(/\*\*(.*?)\*\*/g,`<strong>$1</strong>`).replace(/\*(.*?)\*/g,`<em>$1</em>`).replace(/__(.*?)__/g,`<u>$1</u>`).replace(/~~(.*?)~~/g,`<del>$1</del>`).replace(/`([^`]+)`/g,`<code class="em_discord_code">$1</code>`):`Tu mensaje se mostrará aquí... 😊`,C=()=>{let t=e(`#emTextarea`).val();e(`#em_whatsapp_bubble_text`).html(x(t)),e(`#em_discord_text`).html(S(t));let n=new Date().toLocaleTimeString([],{hour:`2-digit`,minute:`2-digit`});e(`#em_whatsapp_time`).text(n),e(`#em_discord_time`).text(`Hoy a las ${n}`)},w=()=>{let t=e(`#emTextarea`).val().length;e(`#em_char_count`).text(`${t} caracteres`)},T=(t=``)=>{let n=[];n=d===`recientes`?m():l[d]||[],t&&(n=Object.values(l).flat().filter(e=>e.includes(t)));let r=e(`.em_grid`);if(n.length===0){r.html(`<div class="em_empty">No hay emojis disponibles</div>`);return}r.html(n.map(e=>`
  ...
  </div>`},D=async()=>{O(),a(),f=m(),T(),w(),C();let t=(e,t)=>{let n;return(...r)=>{clearTimeout(n),n=setTimeout(()=>e(...r),t)}},n=t(e=>T(e),250),i=t(e=>{v(e),C()},300);e(document).on(`click.em`,`.em_emoji`,async function(t){let n=e(this).data(`emoji`);b(n,t.pageX,t.pageY),await y(n),g(n);let i=e(`#emTextarea`),a=i[0].selectionStart,o=i.val(),s=o.slice(0,a)+n+o.slice(a);i.val(s),i[0].selectionStart=i[0].selectionEnd=a+n.length,i.focus(),v(s),w(),C(),e(this).addClass(`copied`),setTimeout(()=>e(this).removeClass(`copied`),250),r(`${n} Copiado!`,`success`,1e3)}).on(`click.em`,`.em_category_tab`,function(){let t=e(this).data(`cat`);d=t,e(`.em_category_tab`).removeClass(`active`),e(this).addClass(`active`),e(`.em_cat_label`).html(`
        <i class="fas ${c[t].icon}"></i>
        <span style="font-weight: 800; font-size: 12px; margin-left: 8px;">${c[t].label}</span>
      `),e(`#emSearch`).val(``),T()}).on(`click.em`,`.em_preview_tab`,function(){let t=e(this).data(`preview-type`);e(`.em_preview_tab`).removeClass(`active`),e(this).addClass(`active`),t===`whatsapp`?(e(`#em_screen_whatsapp`).removeClass(`dpn`),e(`#em_screen_discord`).addClass(`dpn`)):(e(`#em_screen_whatsapp`).addClass(`dpn`),e(`#em_screen_discord`).removeClass(`dpn`))}).on(`click.em`,`.em_template_btn`,function(){let t=u[e(this).data(`key`)];t&&(e(`#emTextarea`).val(t.text).focus(),v(t.text),w(),C(),r(`Plantilla insertada 📝`,`success`,1500))}).on(`input.em`,`#emSearch`,function(){n(e(this).val().trim())}).on(`input.em`,`#emTextarea`,function(){i(e(this).val()),w()}).on(`click.em`,`#emCopiarTodo, #emCopiarTexto`,async function(t){t.preventDefault();let n=e(`#emTextarea`).val();if(!n){r(`Bloc de notas vacío`,`warning`,1200);return}await y(n),r(`¡Mensaje copiado al portapapeles! 📋`,`success`,1500)}).on(`click.em`,`#emLimpiar`,function(t){t.preventDefault(),e(`#emTextarea`).val(``),v(``),w(),C(),r(`Notepad limpiado 🗑️`,`success`,1200)}).on(`click.em`,`#emInsertarEnter`,function(){let t=e(`#emTextarea`),n=t[0].selectionStart,r=t.val(),i=r.slice(0,n)+`
