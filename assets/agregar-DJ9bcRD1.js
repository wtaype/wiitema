import{n as e}from"./vendor-BuoCFfzO.js";import{S as t,r as n,u as r,w as i}from"./widev-o5MGI85p.js";import{_ as a,b as o,g as s,m as c,n as l,o as u,r as d,u as f}from"./firebase-CcGrcE7k.js";import{n as p,t as m}from"./firebase-Dp4rAqD-.js";var h=`wiAudios`,g=`wiImg`,_=()=>r(`wiSmile`)?.email||m.currentUser?.email||``,v=async e=>{let t=_();return t?(await u(f(s(p,e),c(`email`,`==`,t)))).docs.map(e=>({id:e.id,...e.data()})).sort((e,t)=>(t.creado?.seconds||0)-(e.creado?.seconds||0)):[]},y=async(e,t,n)=>{let r=_();return r?(await l(s(p,e),{email:r,titulo:t,src:n,creado:o(),actualizado:o()})).id:null},b=async(e,t)=>{await d(a(p,e,t))},x=()=>v(h),S=()=>v(g),C=(e,t)=>`
...
</div>`,E=async()=>{if(!_())return n(`Inicia sesión para gestionar recursos`,`warning`);let r=async()=>{let t=await v(h);e(`#agAudList`).html(w(t,h))},i=async()=>{let t=await v(g);e(`#agImgList`).html(w(t,g))};await Promise.all([r(),i()]),e(document).on(`click.ag`,`#agAudBtn`,async function(){let i=e(`#agAudTit`).val().trim(),a=e(`#agAudSrc`).val().trim();if(!i||!a)return n(`Completa título y URL`,`warning`);t(this,!0);try{await y(h,i,a),e(`#agAudTit, #agAudSrc`).val(``),await r(),n(`Audio guardado 🎵`,`success`)}catch(e){console.error(e),n(`Error al guardar`,`error`)}finally{t(this,!1)}}),e(document).on(`click.ag`,`#agImgBtn`,async function(){let r=e(`#agImgTit`).val().trim(),a=e(`#agImgSrc`).val().trim();if(!r||!a)return n(`Completa título y URL`,`warning`);t(this,!0);try{await y(g,r,a),e(`#agImgTit, #agImgSrc`).val(``),await i(),n(`Imagen guardada 🖼️`,`success`)}catch(e){console.error(e),n(`Error al guardar`,`error`)}finally{t(this,!1)}}),e(document).on(`click.ag`,`.ag_copiar`,function(){let t=e(this).closest(`.ag_item`).find(`.ag_item_src`).attr(`title`);navigator.clipboard.writeText(t),n(`URL copiada`,`success`)}),e(document).on(`click.ag`,`.ag_eliminar`,async function(){let t=e(this).closest(`.ag_item`),r=t.data(`id`),i=t.data(`col`);t.css(`opacity`,`.4`);try{await b(i,r),t.slideUp(200,()=>t.remove()),n(`Eliminado`,`info`)}catch(e){console.error(e),t.css(`opacity`,`1`),n(`Error`,`error`)}})},D=()=>{e(document).off(`.ag`)};export{D as cleanup,E as init,x as misAudios,S as misImagenes,T as render};